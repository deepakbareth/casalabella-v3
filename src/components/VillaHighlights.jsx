...
export default function VillaHighlights() {
  ...
  const highlights = [
    ...
    {
      icon: <BedDouble className="w-5 h-5 text-[#E57A55]" />,
    ...
    {
      icon: <Bath className="w-5 h-5 text-[#E57A55]" />,
    ...
    <section id="highlights" className="relative min-h-screen w-full flex flex-col lg:flex-row bg-[#F7F5F0] text-[#2D332F] overflow-hidden py-20 lg:py-24 px-5">
      ...
      <div className="w-full lg:w-[55%] flex flex-col justify-center px-8 sm:px-16 lg:px-24 py-12 lg:py-0 z-10">
        ...
        <h2 className="font-sans text-5xl sm:text-6xl lg:text-[80px] font-black uppercase tracking-tighter leading-[0.85] mb-8">
          Refined <br />
          <span className="text-[#E57A55]">Details</span>
        </h2>
        ...
        <div className="flex flex-col w-full border-t border-[#E3E0D8] mt-4">
          {highlights.map((item, idx) => (
            <button
              ...
              className={`flex items-start text-left py-6 border-b border-[#E3E0D8]/60 transition-all duration-300 group cursor-pointer ${
                activeTab === idx ? 'pl-4' : 'hover:pl-2'
              }`}
            >
              {/* Icon Accent */}
              <div className={`p-2.5 rounded-full border transition-all duration-300 mr-5 mt-0.5 ${
                activeTab === idx 
                  ? 'border-[#E57A55] bg-[#E57A55]/10' 
                  : 'border-[#E3E0D8] bg-transparent group-hover:border-[#E57A55]/60'
              }`}>
                {item.icon}
              </div>
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between gap-4 mb-1">
                  <h3 className={`font-sans text-xl font-bold uppercase tracking-tight transition-colors duration-300 ${
                    activeTab === idx ? 'text-[#E57A55]' : 'text-[#2D332F]/80 group-hover:text-[#2D332F]'
                  }`}>
                    {item.title}
                  </h3>
                ...
                <p className={`font-sans text-sm sm:text-base text-[#5A605C] transition-all duration-500 overflow-hidden leading-relaxed ${
                  activeTab === idx ? 'max-h-24 opacity-100 mt-2' : 'max-h-0 opacity-0 pointer-events-none'
                }`}>
                  {item.desc}
                </p>
              </div>
