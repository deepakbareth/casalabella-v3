        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {suites.map((suite, idx) => (
            ...
            >
              ...
              <p className="font-sans text-xs text-[#5A605C] leading-relaxed mb-6">
                {suite.details}
              </p>
              ...
              <div className="flex flex-wrap gap-2">
                {suite.features.map((feat, fIdx) => (
                  <span 
                    key={fIdx} 
                    className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-[#FFF0E4]/60 border border-[#E3E0D8] rounded-full text-[10px] font-semibold text-[#5A605C] tracking-wide"
                  >
                    {feat.includes("Bed") && <Bed className="w-3 h-3 text-[#ff6e00]" />}
                    {feat.includes("Bath") && <Bath className="w-3 h-3 text-[#007979]" />}
                    {feat.includes("Terrace") || feat.includes("Balcony") || feat.includes("Access") ? <Compass className="w-3 h-3 text-[#007979]" /> : null}
                    {!feat.includes("Bed") && !feat.includes("Bath") && !feat.includes("Terrace") && !feat.includes("Balcony") && !feat.includes("Access") && <UserCheck className="w-3 h-3 text-[#ff6e00]" />}
                    {feat}
                  </span>
                ))}
              </div>
